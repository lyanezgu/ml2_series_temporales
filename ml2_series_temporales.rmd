...
Con la función str vemos los diferentes tipos de datos que tenemos en nuestro dataset, y observamos que la variable date este en formato chr.
...
Gráfico de la serie temporal de la variable meantemp
...
Gráficamente se puede comprobar que existe estacionalidad y una tendencia creciente.Por tanto, parece que se puede decir que la serie no es estacionaria en media.
...
Estudiamos la estacionariedad en varianza. Dado que obtenemos un valor muy próximo a 1, no realizaremos ningún cambio o transformación Box-Cox para ajustar la varianza y estabilizarla.
...
En los pasos previos hemos detectado la necesidad de realizar una diferencia regular y ninguna estacional. Por tanto, comenzaremos con el ajuste de un modelo SARIMA (0,1,0)x(0,0,0)7, sobre la serie.

```{r}
fit <- temp_train_ts %>%
  model(arima = ARIMA(meantemp ~ pdq(0,1,0) + PDQ(0,0,0, period = 12)))
...
```{r}
fit11 <- temp_train_ts %>%
  model(arima = ARIMA(meantemp ~ pdq(1,1,1) + PDQ(0,1,1, period = 7)))
fit11 %>% my_tsresiduals(lag_max = 36)
...
```{r}
#fit2 <- temp_train_ts %>%
 # model(arima = ARIMA(meantemp ~ pdq(2,1,1) + PDQ(2,0,1, period = 7)))
...
```{r}
fit2 <- temp_train_ts %>%
  model(arima = ARIMA(meantemp ~ pdq(3,1,1) + PDQ(2,1,0, period = 7)))
fit2 %>% my_tsresiduals(lag_max = 36)
...
```{r}
pvals_ma = autocorrelation_test_plot(aug_ma, dof = 4, m = 7,  h = 5, alpha = 0.05)
pvals_ma
...
```{r}
summary(lm(std_resid~mean_resid, log_log_ma))

```

### 3.3.2. Residuos modelo automático
...
### 3.4.1. Predicción modelo manual
...
bind_rows(test_err_ma, resids_ma) %>% dplyr::select(Evaluation, everything())
...
```{r}
h_ma <- dim(temp_test_ts)[1]

demanda_plot_ma <- temp_total_ts %>% filter(date>last(temp_train_ts$date)-14 & date< last(temp_train_ts$date) + 7 )

fit2 %>%
  forecast(h=7) %>%
  autoplot(demanda_plot_ma)

```

### 3.4.2. Predicción modelo automático
