...
#tsdata <- ts(temp_train$meantemp,start=c(2013,1),frequency=365)
#plot(tsdata)
#head(tsdata)
...
#autoarima_model<- auto.arima(y = tsdata)
#autoarima_model
```
...
#ajuste del modelo automatica
#library(forecast)
#pred_auto <-forecast(autoarima_model)
#autoplot(pred_auto)
```

### 3.2.3. Selección del modelo final
...
Antes de nada, unimos los dataset de train y test que teníamos separados para crear uno total
...
Comenzamos a realizar la predicción con el modelo final
...
El dataset con los datos de test lo pasamos a tsibble
...
Convertimos la serie estacional a TS
...
```{r}
ts_naive <- window(ts_train_ts,start=c(2013,1),end=c(2016,12))
...
ts_naive_forcast <- snaive(ts_naive ,h=365)
