...
fit %>% my_tsresiduals(lag_max = 56)

```

```{r}
fit11 <- temp_train_ts %>%
  model(arima = ARIMA(meantemp ~ pdq(2,1,2) + PDQ(0,0,0, period = 7)))
fit11 %>% my_tsresiduals(lag_max = 56)
...
```{r}
aug_ma %>% features(.resid, ljung_box, lag=24, dof=2)

```

```{r}
pvals_ma = autocorrelation_test_plot(aug_ma, m = 12, dof = 2, h = 2, alpha = 0.05)
